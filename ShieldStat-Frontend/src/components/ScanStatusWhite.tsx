...
interface ScanStatusWhiteProps {
  domain: string;
}
...
export const ScanStatusWhite: React.FC<ScanStatusWhiteProps> = ({ domain }) => {
  ...
  const stages = [
    { id: 'dns', label: 'DNS Reconnaissance', icon: <Globe size={18} />, description: 'Analyzing name server records and zone files' },
    { id: 'http', label: 'HTTP & Technology', icon: <Cpu size={18} />, description: 'Fingerprinting web server and stack components' },
    { id: 'ssl', label: 'SSL/TLS Analysis', icon: <Lock size={18} />, description: 'Evaluating encryption strength and certificate chain' },
    { id: 'headers', label: 'Security Headers', icon: <ShieldCheck size={18} />, description: 'Checking CSP, HSTS, and X-Frame-Options' },
    { id: 'discovery', label: 'Subdomain Discovery', icon: <Search size={18} />, description: 'Mapping organizational attack surface' },
    { id: 'email', label: 'Email Security', icon: <Mail size={18} />, description: 'Verifying SPF, DKIM, and DMARC policies' },
    { id: 'intel', label: 'Reputation & Intel', icon: <Activity size={18} />, description: 'Checking blacklists and threat intelligence feeds' },
    { id: 'ports', label: 'Port Scanning', icon: <Target size={18} />, description: 'Identifying exposed services and vulnerabilities' },
    { id: 'ai', label: 'AI Risk Scoring', icon: <Zap size={18} />, description: 'Synthesizing findings into executive risk score' },
  ];
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div className="space-y-1">
            ...
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">
              Scanning <span className="text-blue-600">{domain}</span>
            </h2>
          </div>
